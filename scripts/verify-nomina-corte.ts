...
async function main() {
...

  {
    ...
    assert(ids.includes(ordenCompletada.id), "orden completada dentro del rango es elegible");
  ...
  let payrollA: { id: string; total_ventas: unknown };
  {
    ...
    assert(Number(payrollA.total_ventas) === 50, "total_ventas de la quincena = solo la orden completada dentro de rango ($50)");

    const includedOrder = await prisma.order.findUnique({ where: { id: ordenCompletada.id } });
    assert(includedOrder?.incluido_en_nomina_id === payrollA.id, "la orden queda sellada con incluido_en_nomina_id");
  }
  ...
  console.log("\n3b) Volver a ver un período ya pagado (page.tsx) muestra la foto histórica, no vacío");
  {
    ...
    assert(ordenesDelPago.length === 1 && ordenesDelPago[0].id === ordenCompletada.id, "el período ya pagado muestra la orden que realmente se pagó, no una lista vacía");
    assert(Number(payrollA.total_ventas) === 50, "el total mostrado para un período pagado viene del PayrollRecord ($50), no de una re-consulta que daría $0");
  }
  ...
  const ordenNueva = await makeOrder({ userId: user.id, total: 75, status: "completada", createdAt: new Date(2099, 0, 20) });
  {
    const elegibles = await prisma.order.findMany({ where: nominaEligibleWhere(rangoB.inicio, rangoB.fin, user.id) });
    const ids = elegibles.map((o) => o.id);
  ...
  console.log("\n6) Re-pagar el mismo período (edición de comisión) no resetea total_ventas a 0");
  {
    ...
    assert(total_ventas === 50, "al re-editar la comisión de un período ya pagado, total_ventas se conserva ($50) en vez de recalcularse a partir de órdenes ya selladas (que darían 0)");
  }
  ...
  console.log("\n7) Deshacer pago revierte todo: orden vuelve a elegible, expense y record se borran");
  {
    ...
    await prisma.$transaction(async (tx) => {
      await tx.order.updateMany({ where: { incluido_en_nomina_id: payrollA.id }, data: { incluido_en_nomina_id: null } });
...
async function cleanup() {
  ...
  await prisma.order.updateMany({ where: { id: { in: createdOrderIds } }, data: { incluido_en_nomina_id: null } });
  await prisma.order.deleteMany({ where: { id: { in: createdOrderIds } } });
  await prisma.payrollRecord.deleteMany({ where: { id: { in: createdPayrollIds } } });
