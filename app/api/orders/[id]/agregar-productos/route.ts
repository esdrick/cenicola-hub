    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (const line of lines) {
        ...
        const unitPrice = Number(
          resolveUnitPrice(variant, pricingMethod, newTotalQty, mayorThreshold, bundleThreshold)
        );
        const subtotal = parseFloat((unitPrice * line.quantity).toFixed(2));
      ...
      for (const item of order.items) {
        const unitPrice = Number(
          resolveUnitPrice(item.variant, pricingMethod, newTotalQty, mayorThreshold, bundleThreshold)
        );
      ...
      // reach embalaje, same as a full cash payment), tienda completes immediately.
      const isFullyCovered = paidTotal >= newTotalUsd - 0.01;
